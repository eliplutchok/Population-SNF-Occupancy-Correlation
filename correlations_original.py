...
def compute_correlation(args):
    ...
    try:
        ...
        if len(ratio) > 1:
            corr_coef, p_value = pearsonr(ratio, nh_ratio)
...
def perform_analysis(population_type):
    ...
    # Sort results based on absolute correlation coefficient
    ...
    print(f"\nTop {top_n} combinations for {population_type}:")
    for i, result in enumerate(top_results, start=1):
        age_group_names = [age.replace(f'Count!!SEX AND AGE!!{population_type}!!', '') for age in result[3]]
        print(f"{i}. Corr Coef: {result[1]:.4f}, P-value: {result[2]:}, Age Group2: {age_group_names}")
...
def compute_gender_ratio_correlation(args):
    ...
    try:
        ...
        # Mask invalid values
        valid_mask = ~np.isnan(ratio) & ~np.isnan(nursing_home_ratio) & (male_sum != 0)
        ratio = ratio[valid_mask]
        nh_ratio = nursing_home_ratio[valid_mask]
...
def perform_gender_ratio_analysis():
    ...
    print(f"\nBest correlation coefficient for Female to Male Ratios: {best_result[1]:.4f}")
    print(f"P-value: {best_result[2]:}")
    print(f"With age groups: {list(best_result[3])}")

    # Display the top 10 results
    print(f"\nTop {top_n} combinations for Female to Male Ratios:")
    for i, result in enumerate(top_results, start=1):
        age_group_names = list(result[3])
        print(f"{i}. Corr Coef: {result[1]:.4f}, P-value: {result[2]:}, Age Groups: {age_group_names}")
