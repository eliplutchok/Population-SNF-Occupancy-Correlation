...
OUTPUT_FILENAME_TEMPLATE = 'top_result_v2_{population_type}.json'
...
def perform_analysis(population_type):
    ...
    # Calculate the ratio of total nursing home residents to Group1 population
    df['NursingHomeResidents_to_Group1_Ratio'] =(PERC_OF_RESIDENTS_THAT_ARE_IN_GROUP1 * df['total_residents']) /  df['Group1_Sum']
    ...
    # Use the new ratio with Group1_Sum as the denominator
    nursing_home_ratio = df['NursingHomeResidents_to_Group1_Ratio'].values
    ...
    def parallel_compute():
        ...
        args_list = [
            (combo, age_group_values, df['Group1_Sum'].values, nursing_home_ratio) for combo in combinations_list
        ]
    ...
    for i, result in enumerate(top_results, start=1):
        ...
        print(f"{i}. Corr Coef: {result[1]:.4f}, P-value: {result[2]:.6f}, Age Group2: {age_group_names}")
    ...
    if top_results:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ratio = np.where(Group2_Sum != 0, df['Group1_Sum'] / Group2_Sum, np.nan)
...
def perform_gender_ratio_analysis():
    ...
    for i, result in enumerate(top_results, start=1):
        ...
        print(f"{i}. Corr Coef: {result[1]:.4f}, P-value: {result[2]:.6f}, Age Groups: {age_group_names}")
